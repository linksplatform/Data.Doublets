...
int main() {
    ...
    const uint64_t restriction = {1};
    const uint64_t substitution = {1, 1, 1};
    UInt64Links_Update(uint64FfiStorage, &restriction, 1, &substitution, 3, updateHandler);
    printf("Updated link: %lu : %lu -> %lu \n", updatedLink.Index, updatedLink.Source, updatedLink.Target);
    UInt64Links_Each(uint64FfiStorage, &restriction, 1, eachHandler);
    printf("Found link after update: %lu : %lu -> %lu \n", foundLink.Index, foundLink.Source, foundLink.Target);
    UInt64Links_Delete(uint64FfiStorage, &restriction, 1, deleteHandler);
