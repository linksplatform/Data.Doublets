...
{
    public unsafe class UInt64LinksTargetsRecursionlessSizeBalancedTreeMethods : public UInt64LinksRecursionlessSizeBalancedTreeMethodsBase
    {
        public: UInt64LinksTargetsRecursionlessSizeBalancedTreeMethods(LinksConstants<std::uint64_t> constants, RawLink<std::uint64_t>* storage, LinksHeader<std::uint64_t>* header) : base(constants, storage, header) { }
        ...
        public: std::uint64_t GetSize(std::uint64_t node) { return Links[node].SizeAsTarget; }

        public: void SetSize(std::uint64_t node, std::uint64_t size) { Links[node].SizeAsTarget = size; }
