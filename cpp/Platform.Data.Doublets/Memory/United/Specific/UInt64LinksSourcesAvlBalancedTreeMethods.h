namespace Platform::Data::Doublets::Memory::United::Specific
{
    public unsafe class UInt64LinksSourcesAvlBalancedTreeMethods : public UInt64LinksAvlBalancedTreeMethodsBase
    {
        ...
        public: std::uint64_t GetSize(std::uint64_t node) override { return this->GetSizeValue(Links[node].SizeAsSource); }

        public: void SetSize(std::uint64_t node, std::uint64_t size) override { this->SetSizeValue(ref Links[node].SizeAsSource, size); }

        public: bool GetLeftIsChild(std::uint64_t node) override { return this->GetLeftIsChildValue(Links[node].SizeAsSource); }

        public: void SetLeftIsChild(std::uint64_t node, bool value) override { this->SetLeftIsChildValue(ref Links[node].SizeAsSource, value); }

        public: bool GetRightIsChild(std::uint64_t node) override { return this->GetRightIsChildValue(Links[node].SizeAsSource); }

        public: void SetRightIsChild(std::uint64_t node, bool value) override { this->SetRightIsChildValue(ref Links[node].SizeAsSource, value); }

        public: std::uint8_t GetBalance(std::uint64_t node) override { return this->GetBalanceValue(Links[node].SizeAsSource); }

        public: void SetBalance(std::uint64_t node, std::uint8_t value) override { this->SetBalanceValue(ref Links[node].SizeAsSource, value); }
